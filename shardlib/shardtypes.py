        ...
        if name in v:
            if v[name] != size:
                raise TypeCheckError(
                    f"dimension {name}: expected {v[name]}, got {size}"
                )
        else:
            v[name] = size
...
#### Shape checking
def _partition_spec_equiv(
    lhs: jax.sharding.PartitionSpec, rhs: jax.sharding.PartitionSpec
) -> bool:
    if len(lhs) < len(rhs):
        lhs, rhs = rhs, lhs
        ...
        def cb(actual):
            if isinstance(actual, jax.sharding.SingleDeviceSharding):
                if any(dim_spec.sharding for dim_spec in shape_spec.dims):
                    raise TypeCheckError(
                        f"is fully replicated, but expected {expected_spec} is not"
                    )
            elif not isinstance(actual, jax.sharding.NamedSharding):
                if isinstance(actual, jax.sharding.Sharding):
                    raise TypeCheckError(
                        f"is SPMD-sharded but no axis names are available. Use `with Mesh(...):` to provide axis names for type checking."
                    )
                else:
                    raise TypeCheckError(
                        f": unexpected object when checking sharding: {actual}"
                    )
...
class Array:
    ...
    def __class_getitem__(cls, x):
        ...
        if dataclasses.is_dataclass(input_cls):
            ...
            extended_cls = make_dataclass(
                input_cls.__name__, extended_fields, bases=(input_cls,)
            )
...
def make_partition_specs(cls):
    ...
    if origin is tuple:
        ...
        if len(args) != 1 or (type(args[0]) is not str and type(args[0]) is not bytes):
            raise ValueError(
                f"Type annotation {cls} should be <dtype>[<shape string>], got {cls}"
            )
