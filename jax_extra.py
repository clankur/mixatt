    ...
    for field in fields(cls):
        ...
        if hasattr(field.type, "__origin__") and field.type.__origin__ is Union:
            ...
            try:
                ...
                raise ValueError(
                    f"Expected {field.type} for {cls.__name__}.{field.name}, got {type(field_value)} instead."
                )
...
def explicit_activation_checkpointing(f):
    ...
    return jax.ad_checkpoint.checkpoint(
        f, policy=jax.checkpoint_policies.save_any_names_but_these()
    )
